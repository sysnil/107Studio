...
if (!empty($username) && !empty($password)) { //建立连接
    $conn = mysqli_connect('localhost', 'root', '123456', 'user'); //准备SQL语句
    ...
    { //选中“记住我”
        ...
        header("Location:0loginsucc.php"); //关闭数据库,跳转至loginsucc.php
        mysqli_close($conn);
    }
    else 
    { 
        //用户名或密码错误，赋值err为1
        header("Location:0login.php?err=1");
    }
} else { //用户名或密码为空，赋值err为2
    header("Location:0login.php?err=2");
} ?>
